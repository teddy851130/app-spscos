...
**자동 발굴 첫 사이클 완료 + 파이프라인 4개 버그 발견.** Claude가 Apollo MCP로 15개사/26명 발굴 → CSV 생성 → 파이프라인 실행 → 전체 4개 버그 표면화. Perplexity 키 갱신으로 인텔은 정상화됐으나 race condition/team 필터 누락 등 코드 fix 필요. **상세는 `project_sps_pipeline_bugs.md`** (PR16 이월 항목).

**다음 단계**: Teddy가 현재 DB로 직접 테스트 진행 → 전체 수정 내용 정리 후 다음 세션에서 공유 → PR16 진행.
