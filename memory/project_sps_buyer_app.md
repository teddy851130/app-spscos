...
---
## 현재 단계 (2026-04-21 심야 기준)

**Sprint04 Day 2 종료.** 오늘 하루 PR16 + PR17 + PR17.1 + PR17.2 + PR18 + PR19 + PR21+22 총 6커밋 배포 + run-pipeline Edge Function 재배포 완료. **아키텍처 큰 방향 전환** 확정: 배치 자동 초안 경로(직원D/E) 완전 폐기 → Buyers DB 수동 경로(EmailComposeModal) 단일화. "오늘 보낼 메일" 페이지 제거(-775줄). 팔로업 버튼 email_count 5단계 컬러 계단(≥4 "보관" disabled). 에이전트 스펙 md 6개 신규 + CSV 스킵 사유 UI + 도메인 중복 체크 SQL 병기. **현재 상태 상세는 `project_sps_sprint04.md`**.

**다음 세션 첫 액션**: 토요일(4/25) 1차 발송 **직전** PR0 재측정 (Teddy 자기 앞 2~3통 → Primary 5/5 확인 후 외부 5~10통). 프롬프트 HARD LIMITS 7건 + 5줄 서명이 실제 Primary 판정에 미치는 영향 실측.

**남은 블로커 / 대기**:
1. **PR0 재측정** — 토 오전 필수. Primary < 5/5면 외부 발송 연기 + PR17/18 재튜닝
2. **통합 웹사이트 런칭** — 바이어 노출용 URL(`app.spscos.com` 교체). 별도 프로젝트
3. **Tier 2 자동 승격** — 2026-04-26경 예정(7일 요건). 현재 race condition 부분 해소됨(PR16) 추가 관찰
