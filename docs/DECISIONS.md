...
### 이유
- PR11/PR12 프롬프트 v4가 모든 첫 메일 P.S.에 `https://spscos.com/` 삽입 중인데 현재는 클릭 이벤트 자체가 수집 안 됨 → Teddy가 "관심 리드 수동 팔로업 타이밍을 놓침" 지적.
- 외부 추적 서비스(Mailgun/Hunter/bit.ly) 모두 월 구독 + 프라이버시/도메인 평판 리스크. 자체 redirect가 가장 단순 + 통제 가능.
- HMAC secret 방식은 Teddy가 키 관리 부담 + 실제 필요 없음 — URL 위조해도 DB lookup 단계에서 폐기되므로 12자 hex UNIQUE로 충분 (충돌 공간 2^48).
- Pipedrive Activity는 이미 BCC(`spscos@pipedrivemail.com`)로 Deal/Person 연동됨 → Person 검색 후 Activity append만 추가하면 영업 히스토리 완성.

### 대안 기각
- **옵션 A (자동 팔로업 메일)**: 범위 외로 분리. Teddy가 "공들인 수동 팔로업"을 선호.
- **HMAC 서명 토큰**: secret env 관리 필요 + 실제 보안 이득 없음. DB UNIQUE 토큰이 더 단순.
- **trigger 기반 상태 갱신**: 테스트·디버깅 어려움. route.ts 코드에서 명시적 UPDATE.
- **pipeline_logs 실패 기록**: `agent` CHECK가 A~F만 허용 + `job_id` FK 필수 → 클릭 이벤트는 pipeline_job 밖이라 부적합. click_events 자체 컬럼이 적절.
...
### 비용
- Vercel Function 호출 — 무료 플랜 포함 범위 내.
- Pipedrive API — 기존 구독에 포함.
- DB 용량 — click_events 1건당 ~200 bytes.

**관련 파일**:
- `supabase/migrations/011_click_tracking.sql` (+ rollback)
- `app/go/[token]/route.ts`, `app/lib/supabaseAdmin.ts`, `app/lib/pipedrive.ts`, `app/components/InterestedLeadsWidget.tsx`
- `supabase/functions/run-pipeline/index.ts` (v34), `generate-draft/index.ts` (v18), `validate-draft/index.ts` (v11)
- `app/components/Dashboard.tsx`, `MailQueue.tsx` (도메인 체크 동기화)
- 커밋 `e4ff31e` (PR13)
...
## ADR-034: 스팸 점수 라벨 통일 + 로컬 체크 엄격화 + 편집 진입 시 DB 점수 보존

**날짜**: 2026-04-19
**결정**: 스팸 관련 UI 3가지 혼동/버그를 동시 해소. (1) "스팸: X/10" 라벨이 "스팸 위험 점수"인지 "스팸 안전 점수"인지 불분명 → "안전도: X/10"으로 통일. (2) MailQueue `checkSpamClient`가 규칙 위반 1건만 있어도 8점을 반환하는 자기모순 → 위반 있으면 score ≤ 5 강제. (3) "수정" 진입 즉시 로컬 재검사해 DB 점수를 덮어쓰는 UX 버그 → 본문 변경 전까지 DB 점수 보존.

### 이유
- Teddy 재현 시나리오: Sisley Paris 초안 DB 안전도 3.0/10(빨강·flag) → "수정" 클릭만 했을 뿐인데 즉시 "스팸 위험: 낮음(8/10) — 대문자 연속 감지" 표시. (a) 라벨이 "스팸"이면 낮을수록 위험인지 안전인지 헷갈림. (b) "위반 감지됐는데 위험 낮음"이라는 자기모순. (c) DB 3.0이 로컬 8.0으로 교체돼 사용자가 안심.
- 라벨 혼동은 ADR-001에서 정한 "10=안전, 1=위험" 스케일을 UI에 충실히 반영하지 못한 결과. "스팸"은 중립 명사, 해석 주체가 필요해 오해 소지 큼.
- 로컬 체크가 관대한 건 Claude rubric 없이 5가지 규칙만 보기 때문 — 이건 구조적 한계지만, 위반이 하나라도 있을 때 8점 주는 건 명백한 수치 오류.

### 변경 범위
1. **MailQueue.tsx `checkSpamClient`**: `issues.length === 0 ? 10 : Math.max(1, Math.min(5, 6 - issues.length × 2))`. 0위반=10, 1위반=5(cap), 2위반=4, 3+=감소. "낮음(≥8)" 구간에 issue>0이 들어갈 수 없음.
2. **라벨**: MailQueue 뱃지·Dashboard flag 섹션 뱃지 모두 "스팸" → "안전도". title 속성에 스케일 설명 추가.
3. **EmailComposeModal flag 뱃지**: "스팸 위험" → "검증 실패 (안전도 X/10)".
4. **편집 진입 시점 보존**: "수정" 클릭 시 `setSpamResult(null)`. 별도 회색 배너 "편집 전 DB 안전도: X/10 (본문 수정 시 로컬 재검사)". 본문 textarea `onChange`에서 `checkSpamClient` 호출 유지.
5. **로컬 결과 박스**: "스팸 위험: 낮음(8/10)" → "안전도: 8/10". issues 없으면 "규칙 통과". 하단에 항상 "※ 로컬 규칙 기준. 저장 시 서버 Claude 재검증에서 추가로 감점될 수 있음" 문구.

### 대안 기각
- **규칙 위반 있을 때 score=0 또는 1로 강제**: 과도. Claude rubric 없는 로컬 체크가 "절대 안전 아니다"라고 단정하는 건 권한 밖.
- **라벨 "스팸 안전도" (복합)**: "안전도"만으로 충분히 명확, 글자 수 절약.
- **EmailComposeModal pass 뱃지도 "안전도" 라벨로 통일**: "검증 통과"가 상태 + 점수 모두 표현해 이미 명확. 불필요한 변경.

### 한계
- 로컬 체크와 서버 Claude 판정 격차는 여전히 존재 (구조적). 사용자는 "로컬 5=경고" 봤어도 서버가 3을 줄 수 있음 → 문구로 안내만.

**관련 파일**:
- `app/components/MailQueue.tsx` (checkSpamClient 공식 + 뱃지 + 편집 모드 UX)
- `app/components/Dashboard.tsx` (flag 섹션 뱃지)
- `app/components/EmailComposeModal.tsx` (flag 상태 뱃지)
...
## ADR-047: PR19 팔로업 버튼 UI — email_count 기반 5단계 컬러 계단 + ≥4 보관 disabled
**날짜**: 2026-04-21 (PR19)
**결정**: Buyers.tsx + Dashboard.tsx 팔로업 섹션의 "메일 작성" 버튼을 `email_count` 값에 따라 5단계 라벨·컬러로 자동 전환. 0=첫 발송/회색, 1=1차 팔로업/노랑, 2=2차 팔로업/녹색, 3=3차 팔로업/빨강, ≥4=보관/빨강+opacity60+disabled(tooltip "3차 팔로업 완료. 더 이상 발송하지 않음."). 기존 3단계(첫 발송/팔로업/마지막) 로직 교체. DB 스키마 변경 없음 — `email_count` 증가·`next_followup_at` 갱신은 `increment_email_sent` RPC가 이미 원자적으로 처리(ADR-008).
**이유**: Teddy가 파이프라인의 팔로업 회차를 한눈에 식별할 필요. 3단계(첫 발송/팔로업/마지막)는 1차/2차/3차 구분이 안 돼서 "몇 번째 보낸 사람인지" 구분 불가. 5단계 컬러 계단 — 회색→노랑→녹색→빨강 — 은 진행도와 긴급도를 동시에 시각화. ≥4는 물리적 disabled 처리로 실수 재발송 차단(소프트 워딩보다 안전). "보관" 워딩은 아카이브 의미와 일치하고 "최종 시도"(추가 발송 여지 오해)·"리마인드"(톤 중복) 대비 직관적.
**대안 기각**: (a) 드롭다운 필터로 "1차만 보기" — Teddy 명시 거부, 필터링보다 한 화면 시각화가 파이프라인 파악에 유리. (b) ≥4 버튼을 숨김(display:none) — 바이어 레코드 자체가 사라진 것처럼 보임. disabled 유지 + 라벨 표기가 "여기까지 발송했다"는 이력을 남김. (c) 초록→노랑→빨강 신호등 3단계 — 1차/2차 구분 불가로 현행 3단계와 동일 문제. (d) email_count 대신 status('Contacted')로 분기 — status는 "회신받음" 등으로 팔로업과 독립 전이하므로 부정확.
**관련**: PR19 커밋(이번), `app/components/Buyers.tsx:558-572` + `:677-686`, `app/components/Dashboard.tsx:598-639`, `sprints/Sprint04_Plan_v3.md` PR19 섹션, 기존 ADR-044(Dashboard 팔로업 흡수)·ADR-008(increment_email_sent RPC)
