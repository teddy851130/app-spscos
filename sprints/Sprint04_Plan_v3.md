...
### PR18 — 직원D/E 파이프라인 자동화 제거 + Dashboard 초안 목록 제거 (화~수, Teddy 요청으로 전면 재정의)

**원래 v3 PR18 계획 폐기**: "스팸 자동 재생성 루프 + validate-draft MAX_REGEN=2 fetch 샘플 코드"는 직원D/E가 배치 자동 생성할 때 유효했던 플로우. Teddy 결정(2026-04-21): **배치 자동 초안 생성·스팸 검증 경로를 완전 제거**.

**배경**: 실제 사용 플로우는 "Buyers DB 페이지 → 바이어 인텔 확인 → 국문 초안(직원D 역할) → 번역 save(직원E 역할)" 수동 경로. 직원D/E의 파이프라인 배치 경로는 사용되지 않음(발송 시 EmailComposeModal 경유) + Claude API 비용 낭비. Dashboard "이메일 초안 목록 / 검토 필요 / 인텔 대기" 3개 섹션도 이 자동 경로 결과물 표시용 — 함께 제거.

#### PR18 작업 범위

**A. `run-pipeline/index.ts` 슬림화 — 직원 B → C → F만 남김**:
- `agentD` 함수 전체 제거 (~370줄, L557~L929)
- `agentE` 함수 전체 제거 (~165줄, L930~L1094)
- `SPAM_WORDS` / `SPS_DOMAIN_RE` / `EXTERNAL_LINK_RE` / `checkSpamRules` / `autoFixSpam` 헬퍼 제거 (validate-draft/index.ts에 복사본 유지)
- 메인 핸들러 `agents` 배열에서 D, E 제거
- `agentF` 로그 스캔 대상 `["B","C","D","E"]` → `["B","C"]`
- `agentF`의 직원 D 결과 체크 블록 제거 (email_drafts 생성 0건 체크)
- 파일 상단 주석 "직원 B→C→D→E→F" → "직원 B→C→F"

**B. `Dashboard.tsx` 초안 목록 섹션 3개 제거**:
- state: `emailDrafts` / `flaggedDrafts` / `pendingIntelDrafts` / `previewDraft` 제거
- loadDashboard 내 `email_drafts` 조회 블록 제거
- JSX "이메일 초안 목록" / "검토 필요 (스팸 점수 미달)" / "인텔 데이터 필요" 3개 섹션 제거
- `EmailDraft` import 미사용이면 제거

**C. 검증**:
- `npx tsc --noEmit` 통과
- 파이프라인 1회 실행 → 직원 B→C→F 로그만 (D,E 없음)
- Dashboard에 초안 섹션 없는지
- Buyers 페이지 바이어 인텔 탭에서 국문 초안 → 영문 번역 수동 경로 여전히 작동

**D. 유지되는 것**:
- `generate-draft` Edge Function (수동 경로의 generate_ko + translate_save)
- `validate-draft` Edge Function (EmailComposeModal 저장 재검증)
- `email_drafts` 테이블 (수동 경로에서 INSERT)
- PR17/17.1에서 generate-draft·translate_save·validate-draft에 반영한 톤·서명·SPAM_WORDS·HARD LIMITS 모두 그대로 유효

**ADR-045**: 파이프라인 agentD/E 제거 — 배치 자동 초안 경로 폐기, 초안 생성은 수동 경로(EmailComposeModal) 단일화.

- **DB migration**: 불필요.
- **Edge Function 재배포**: `run-pipeline` **필수**.
- **예상 공수**: 오늘 오후 (~3시간).
- **의존성**: PR17 완료.

---

### [폐기] 원래 v3 PR18 "스팸 자동 재생성 루프"

validate-draft MAX_REGEN=2 / generate-draft 재시도 프롬프트 주입 / normalizeParagraphs 후처리 등은 **배치 자동 초안 경로가 있을 때만 필요**. 수동 경로는 사용자가 저장 시 validate-draft 1회 호출 → flag이면 본인이 다듬어 재저장. 재생성 자동 루프는 인간이 검수하는 수동 경로에 불필요.

아래 원래 v3 PR18 본문 보존 (히스토리용):

#### 원본 v3 추가 1 — validate-draft → generate-draft fetch 샘플 코드 (v2 경로만 명시 → v3 실행 가능 샘플)
...
### PR19 — 팔로업 버튼 UI 재설계 (Buyers 페이지) + 팔로업 회차 계단 (목)

**2026-04-21 신규 요구 (Teddy)**: 기존 드롭다운 필터 폐기. **행별 발송 상태 버튼**으로 팔로업 회차 시각화.

#### 상태 전이 (email_count 기반 컬러 계단)

| email_count | 버튼 라벨 | 컬러 | 의미 |
|---|---|---|---|
| 0 | `첫 발송` | 회색/흰색 | 미발송 바이어 |
| 1 | `1차 팔로업` | 노랑 (`#f59e0b`) | 1회 발송 완료 |
| 2 | `2차 팔로업` | 녹색 (`#22c55e`) | 팔로업 1회 완료 |
| 3 | `3차 팔로업` | 빨강 (`#ef4444`) | 팔로업 2회 완료, 마지막 시도 임박 |
| ≥4 | `보관` (Teddy 워딩 확인) | 빨강 (`#ef4444`) | 3차 팔로업도 발송 완료, 아카이브 |

(최종 라벨 후보: **"보관"** / "최종 시도" / "리마인드" — Teddy 결정 대기.)

#### 구현 범위

**A. `app/components/Buyers.tsx`** — 각 행에 발송 버튼 추가. 기존 "메일 작성" 버튼을 email_count에 따라 라벨·컬러 동적 변경. ≥4이면 disabled + "보관" 텍스트.

**B. `app/components/EmailComposeModal.tsx` onSent 콜백** — 발송 완료 시 `buyers.email_count++` + `last_sent_at=now` + `next_followup_at=now+(Tier1=5d/Tier2=7d)`. 이미 구현돼 있으면 유지.

**C. Dashboard 팔로업 리스트에도 동일 컬러 계단** (PR17.2에서 flatten 완료된 목록).

**ADR-046**: 팔로업 회차 버튼 UI — 발송 카운트 기반 컬러 계단, 드롭다운 필터 폐기.
...
### [폐기] 원 v3 PR19 "팔로업 회차 로직 + 수기 1문장 hybrid 슬롯"

v2에서 계승된 "팔로업 회차별 본문 차별화 + 수기 1문장 hybrid 슬롯" 범위는 PR19-B로 분리하거나 Sprint05로 이월 검토. 우선 버튼 UI부터.
